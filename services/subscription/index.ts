...
export const getSubscriptionUsersPaid = async (dateRange: string, page: number, validityChange: string, debouncedSearch: string) => {
     try {
          const { data } = await api.get(`/subscriptionAccess/paidUsers?search=${debouncedSearch}&tierType=${validityChange}&subscriptionStatus=${validityChange}&dateRange=${dateRange}&page=${page}&limit=10`)
          return data;
     } catch (error: any) {
          toast.error(error?.message)
          console.error(error?.message)
