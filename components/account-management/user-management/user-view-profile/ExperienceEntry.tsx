    ...
    <section className="w-full rounded-xl border p-[1rem] border-border-gray-light bg-surface-DEFAULT mt-5">
      ...
      <div className="">
        {workExperience?.map((entry, index) => (
          <div
            key={index}
            className={`py-5 ${index !== workExperienceData.length - 1
          ...
          >
            <div>
              ...
              <p className="mt-1 text-[0.875rem] text-brand-rating-text-border font-normal">{entry?.companyName}</p>

              <p className="mt-2 text-[0.875rem] font-normal text-brand-primary font-inter">
                {moment(entry?.startDate).format("MMM YYYY")} - {entry?.endDate ? moment(entry?.endDate)?.format('MMM YYYY') : `Present`}
              </p>
            </div>
            ...
            <div className="mt-1.5 flex flex-col">
              <div className="flex">
                {entry?.employmentType?.map((type, index) => {
                  return (
                    <span
                      key={index}
                      className="inline-block bg-gray-200 text-black px-2 py-1 rounded"
                    >
                      {type}
                    </span>)
                })}
              </div>
              <p className="flex-1 text-[0.875rem] leading-[1.75rem] font-normal text-text-secondary">{entry?.location}</p>
              <div
              ...
              >
                ...
                <p
                  className={`flex-1 text-[0.875rem] leading-[1.75rem] font-normal text-text-secondary pb-[1.75rem]`}
