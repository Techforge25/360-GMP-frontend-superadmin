import Link from "next/link";
import { sidebarMenuItems as menuItemsData, sidebarMenuItems } from "@/constants/sidebar/sidebar";
import { useNavigationStore } from "@/store/modulesStore";
import useNavLinks from "@/hooks/useNavLinks";

interface SidebarMenuItemsProps {
  pathname: string;
}

export default function SidebarMenuItems({ pathname }: SidebarMenuItemsProps) {
  const navigations = useNavLinks()
  const type = useNavigationStore((state) => state.type)
  return (
    <div className="flex-1 mt-3 flex flex-col gap-1 px-3 overflow-y-auto custom-scrollbar">
      {type === 'admin' ? (
        <>
          {navigations?.map((item: any, index: number) => {
            const isSelected =
              pathname === item.url || pathname.startsWith(`${item.url}/`);

            return (
              <Link
                key={index}
                href={item.url}
                className={`flex items-center whitespace-nowrap gap-3 px-3 py-3 rounded-lg transition-all duration-200 font-medium ${isSelected
                  ? "bg-brand-primary text-text-inverse"
                  : "text-black hover:bg-brand-primary hover:text-text-inverse"
                  }`}
              >
                <span className="text-[1.05rem]">{item.icon}</span>
                <span className="text-[1rem]">{item.module}</span>
              </Link>
            );
          })}
        </>
      ) : (
        <>
          {sidebarMenuItems?.map((item: any, index: number) => {
            const isSelected =
              pathname === item.path || pathname.startsWith(`${item.path}/`);

            return (
              <Link
                key={index}
                href={item.path}
                className={`flex items-center whitespace-nowrap gap-3 px-3 py-3 rounded-lg transition-all duration-200 font-medium ${isSelected
                  ? "bg-brand-primary text-text-inverse"
                  : "text-black hover:bg-brand-primary hover:text-text-inverse"
                  }`}
              >
                <span className="text-[1.05rem]">{item.icon}</span>
                <span className="text-[1rem]">{item.name}</span>
              </Link>
            );
          })}
        </>
      )}

    </div>
  );
}
