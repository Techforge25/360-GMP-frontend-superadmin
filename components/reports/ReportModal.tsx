...
import { viewBusinessReports, viewCommunityReports, viewJobReports, viewProductReports } from "@/services/reports";
...
const ReportModal = forwardRef<ReportModalRef, ReportModalProps>(
  ({ isOpen, reportType, ReportId, reportModal, onClose }, ref) => {
    ...
    const { data: dataJob, isPending: isPendingJob } = useQuery({
      queryKey: [keys.viewReportJob, reportModal === 'Job'],
    ...
    const { data: dataBusiness, isPending: isPendingBusiness } = useQuery({
      queryKey: [keys.viewReportBusiness, reportModal === 'Business'],
    ...
    const { data: dataProduct, isPending: isPendingProduct } = useQuery({
      queryKey: [keys.viewReportProduct, reportModal === 'Product'],
    ...
    const { data: dataCommunity, isPending: isPendingCommunity } = useQuery({
      queryKey: [keys.viewReportCommunity, reportModal === 'Community'],
      queryFn: () => viewCommunityReports(ReportId),
      staleTime: 0,
      enabled: !!ReportId && reportModal === 'Community'
    });

    const displayData = reportType === 'job' ? dataJob?.data : reportType === 'business' ? dataBusiness?.data : reportType === 'product' ? dataProduct?.data : reportType === 'community' ? dataCommunity?.data : null
    const pendingData = reportType === 'job' ? isPendingJob : reportType === 'business' ? isPendingBusiness : reportType === 'product' ? isPendingProduct : reportType === 'community' ? isPendingCommunity : null
                  ...
                  <div className="flex-1 overflow-y-auto p-6 sm:px-8 space-y-5 custom-scrollbar">
                    <div className="rounded-lg border border-gray-200 p-5 shadow-sm">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        />
                        <div>
                          <h4 className="font-semibold text-gray-900">{displayData?.reportedBy?.fullName}</h4>
                    ...
                    {displayData?.media && (
                      <div className="rounded-lg border border-gray-200 p-5 shadow-sm">
                        ...
                        <div className="space-y-3">
                          {displayData?.media?.map((item: string, index: number) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  className="mr-2 cursor-pointer text-gray-400 transition-colors hover:text-gray-700"
                                >
